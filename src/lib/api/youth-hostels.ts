/**
 * Youth Hostels API Client
 * وحدة بيوت الشباب - API
 */
import api from './client';

export interface YouthHostel {
  id: string;
  name_ar: string;
  name_fr?: string;
  short_name?: string;
...
export interface YouthHostelDetail extends YouthHostel {
...
export const youthHostelsApi = {
  ...
  // Admin endpoints
  async getHostels(): Promise<YouthHostel[]> {
    return api.get('/youth-hostels').then(r => r.data);
  },

  // Public endpoints
  async getPublicHostels(): Promise<YouthHostel[]> {
    return api.get('/youth-hostels/public/hostels').then(r => r.data);
  },

  async getHostel(id: string): Promise<YouthHostelDetail> {
    return api.get(`/youth-hostels/${id}`).then(r => r.data);
  },

  async getPublicHostel(id: string): Promise<YouthHostelDetail> {
  ...
  // Financial Receipts (Regie / Quittances)
